...
object MonthlySchedule {
  ...
  class Builder(
      ...
      def build(): Schedule = {
        validate
        new MonthlySchedule(dayfuncOpt.get,
            workingSchedOpt.getOrElse(WorkingSchedule.any),
            monthCycleOpt.get,
            monthOffsetOpt.get )
      }
  }
...
//  def monthly(day1: Int): Schedule = new MonthlySchedule(new DayOfMonthBounded(day1), WorkingSchedule.any, 1, 0)
