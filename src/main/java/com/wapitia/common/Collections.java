...
public class Collections {
    ...
    public static <K, V> void addToMapOfLists(final Map<K, List<V>> map, final K key, final V value) {
        addToMapOfLists(map, key, value, (final K k) -> new ArrayList<>());
    }
     ...
     */
    public static <K, C> C getOrCreateMapValue(final Map<K, C> map, final K key,
            final Function<K, C> defaultValueProvider) {
        return Optional.ofNullable(map.get(key)).orElseGet(new MapUpdater<>(map, key, defaultValueProvider));
    }
    ...
    static class MapUpdater<K, C> implements Supplier<C> {
         ...
         *                             value at key in the map.
         */
        MapUpdater(final Map<K, C> map, final K key, final Function<K, C> defaultValueProvider) {
            this.map = map;
            this.key = key;
